...
def isAndroidKeySentence(s):
    re_word = re.compile(CHECK_ANDROID_KEY_SENTENCE)
    if re_word.match(s)!=None:
        return True
    else:
        return False
...
def getParseLabelResult(s):
    if isKeySentence(s):
        return parseLabelResult(s)
    else:
        return None

def getParseAndroidLabelResult(s):
    if isAndroidKeySentence(s):
        return parseLabelResult(s)
    else:
        return None
...
if __name__ == '__main__':
    ...
    print(getParseAndroidLabelResult(str4))
