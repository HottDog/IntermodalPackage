...
function M.getCubicBezierTab(x1,y1,x2,y2,retNum)
    ...
    local x4,y4 = 1,1
    local ret = {}
    for t=0,t<=1,1/retNum do
        ret[t] = {}
        ret[t].x = x0*math.pow(1-t,3) + 3*x1*t*math.pow(1-t,2) + 3*x2*math.pow(t,2)*(1-t) + x4*math.pow(t,3)
        ret[t].y = y0*math.pow(1-t,3) + 3*y1*t*math.pow(1-t,2) + 3*y2*math.pow(t,2)*(1-t) + y4*math.pow(t,3)
    end
    return ret
end
--[Comment]
