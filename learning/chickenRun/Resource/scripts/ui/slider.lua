...
Slider = class(Node);
--修正参数
Slider.CORRECTION_VALUE = 20
Slider.s_defaultBgImage = "ui/sliderBg.png";
Slider.s_defaultFgImage = "ui/sliderFg.png";
Slider.s_defaultButtonImage = "common/decoration/choice_icon.png";
...
Slider.ctor = function(self, width, height, bgImage, fgImage, buttonImage, leftWidth, rightWidth, topWidth, bottomWidth)
    ...
    self.m_bg:setEventTouch(self,self.onBackgroundEventTouch2);
    ...
    self.m_button:setSize(30,30)
    self.m_button:setEventTouch(self,self.onEventTouch2);
    
    self.m_width = width;
    self.m_height = height
    self.m_changeCallback = {};
    Slider.setProgress2(self,1.0);
...
Slider.setProgress2 = function(self, progress)
    progress = progress > 1 and 1 or progress;
    progress = progress < 0 and 0 or progress;
    self.m_progress = progress;
    ...
    self.m_fg:setClip(0,self.m_height*self.m_progress,self.m_width,self.m_height);
    if self.m_changeCallback.func then
        self.m_changeCallback.func(self.m_changeCallback.obj,self.m_progress);
    end 
end
...
Slider.onEventTouch2 = function(self, finger_action, x, y, drawing_id_first, drawing_id_current)
    if finger_action == kFingerDown then
        self.m_dragingY = y;
        self.m_button:setColor(128,128,128);
    else 
        ...
        if (bgY <= y) and (self.m_height + bgY >= y) then 
            ...
            notifyChange();
        elseif (y < bgY ) and (self.m_progress > 0) then 
            -- 移动太快，刚跳出边界时，校正下。
            Slider.setProgress2(self,0);
            self.m_dragingY = bgY 
        
            notifyChange();
        elseif (y > self.m_height + bgY ) and (self.m_progress < 1) then 
            -- 移动太快，刚跳出边界时，校正下。
            Slider.setProgress2(self,1);
...
Slider.onBackgroundEventTouch2 = function(self, finger_action, x, y, drawing_id_first, drawing_id_current)
    if finger_action == kFingerDown then
        local bgX, bgY = self:getAbsolutePos();
        local deltaY = y - bgY;
    ...
    self:onEventTouch2(finger_action, x, y, drawing_id_first, drawing_id_current);
end
