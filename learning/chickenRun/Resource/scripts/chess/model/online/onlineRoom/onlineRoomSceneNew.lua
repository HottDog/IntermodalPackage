...
--    --Test
--    self.m_sendSocketBtn = self.m_root_view:getChildByName("Button1");
--    self.m_sendSocketBtn:setOnClick(self,function()
--        self:requestCtrlCmd(OnlineRoomController.s_cmds.send_gift,self.m_upUser:getUid());
--    end);
--    --
    ...
    self.m_up_gift_anim_view = self.m_up_user_icon_bg:getChildByName("up_anim_view");
...
--    self.m_up_user_icon:addChild(self.m_up_user_level_icon); -- 切换 level
    ...
    self.m_down_gift_anim_view = self.m_down_user_icon_bg:getChildByName("down_anim_view");
...
OnlineRoomSceneNew.ready_action = function(self)
    ...
    local roomType = RoomProxy.getInstance():getCurRoomType();
	local isCanAccess = RoomProxy.getInstance():canAccessRoom(roomType,money);
...
OnlineRoomSceneNew.upComeIn = function(self, user, isSynDataComeIn)
    ...
    if isSynDataComeIn and user:getUid()<=0 then
        ...
        if roomType == RoomConfig.ROOM_TYPE_PRIVATE_ROOM then
            self.m_private_change_player_btn:setVisible(RoomProxy.getInstance():isSelfRoom() and self.m_upUser ~= nil);
            self.m_private_invite_friends_btn:setVisible(RoomProxy.getInstance():isSelfRoom() and self.m_upUser == nil);
        else
            self.m_private_change_player_btn:setVisible(false);
            self.m_private_invite_friends_btn:setVisible(false);
        end
...
--                delete(self.m_countdownAnim);
--                self.m_countdownAnim = nil;
            CountDownAnim.stop(self.m_down_turn);
...
--                delete(self.m_countdownAnim);
--                self.m_countdownAnim = nil;
                CountDownAnim.stop(self.m_up_turn);
...
OnlineRoomSceneNew.runTimout = function(self)
    ...
    CountDownAnim.play(self.m_down_turn,time);
--    if self.m_countdownAnim then
...
--        delete(self.m_countdownAnim);
        CountDownAnim.play(self.m_up_turn,time);
...
--    self.m_countdownAnim:start();
    if self.timeout_pause and self.m_upUser and self.m_upUser:getFlag() == self.m_move_flag then
        CountDownAnim.pause()--self.m_up_turn,time);
--        self.m_countdownAnim:pause();
    end
end
...
--            end
--        end
    elseif data.status == STATUS_TABLE_FORESTALL or data.status == STATUS_TABLE_HANDICAP then
...
OnlineRoomSceneNew.undoAction = function(self)
	if self:isEnableUndo() or self.isUndoAble then
	    ...
	    if roomInfo and roomInfo.undomoney and roomInfo.undomoney > 0 then
	        message = "确认向对方支付"..tostring(roomInfo.undomoney).."金币请求悔棋吗？" ;
        end
...
OnlineRoomSceneNew.chess = function(self)
    local param = self.m_stat_place;
--	local event_info = DOWN_MODEL_DAPU_BTN .. "," .. self.m_stat_place;
--	on_event_stat(event_info); --事件统计
    StatisticsManager.getInstance():onCountToUM(DOWN_MODEL_DAPU_BTN,param);
	self.m_board_menu_dialog:dismiss();
...
OnlineRoomSceneNew.surrender = function(self)
    if not self:checkCanSurrender() then return end
...
OnlineRoomSceneNew.onSetTime = function(self,time_out)
    ...
    local roomType = RoomProxy.getInstance():getCurRoomType();
    local roomConfig = RoomConfig.getInstance():getRoomTypeConfig(roomType);
    local roomLevel = (roomConfig and tonumber(roomConfig.level)) or 0;
    if not self.m_roomSetTimeDialog then
        self.m_roomSetTimeDialog = new(RoomFriendSetTime,roomLevel);
    end
...
OnlineRoomSceneNew.onSetTimeShow = function(self,data)
    ...
    if not self.m_roomOtherSetTimeDialog then
        self.m_roomOtherSetTimeDialog = new(RoomFriendSetTimeShow,roomLevel);
    end
...
--更新观战dialog列表
function OnlineRoomSceneNew.onUpdateDialogView(self, data)
    if RoomProxy.getInstance():getCurRoomType() == RoomConfig.ROOM_TYPE_WATCH_ROOM then
        if OnlineRoomSceneNew.IS_NEW then
            self.mModule:onUpdateWatchDialogView(data);
        else
...
function OnlineRoomSceneNew.onSendGiftReturn(self,data)
    ...
    if roomType == RoomConfig.ROOM_TYPE_WATCH_ROOM then
        ...
        if self.mModule and user_icon then
            GiftModuleAnimManager.playAnim(self,data.gift_type,self.m_watch_gift_start_view,user_icon,angle)
        end
        return
    end
    --播放动画
    GiftModuleAnimManager.playAnim(self,data.gift_type,self.m_down_gift_anim_view,self.m_up_gift_anim_view,270)
--    local anim = new(EggAnim,self.m_head_icon,self.m_my_sexual,0);
--    anim:play();
end
    ...
    if roomType == RoomConfig.ROOM_TYPE_WATCH_ROOM then
        ...
        elseif gift_target_id ~= upUid and gift_target_id ~= downUid then
            ...
            if animList and animList == 0 and user_icon then
                if self.mModule then
                    GiftModuleAnimManager.playAnim(self,data.gift_type,start_gift_view,user_icon,angle,halfMode)
                end
            end
        end
    else
        if gift_send_id == upUid and gift_target_id == UserInfo.getInstance():getUid() then
            ...
            if animList and animList == 0 then
                GiftModuleAnimManager.playAnim(self,data.gift_type,self.m_up_gift_anim_view,self.m_down_gift_anim_view,90)
            end
        end
    end
end
