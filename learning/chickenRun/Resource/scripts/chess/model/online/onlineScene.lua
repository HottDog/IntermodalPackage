...
--    self.m_radioButtonGroup:setSize(500,150);
--    self.m_radioButtonGroup:setAlign(kAlignCenter);
--    self.m_root:addChild(self.m_radioButtonGroup)


--    local node1 = new(RadioCheckButton);
--    node1:setPos(50,0);
--    node1:setAlign(kAlignLeft);
--    self.m_radioButtonGroup:addChild(node1)
--    local node2 = new(RadioCheckButton);
--    node2:setPos(300,0);
--    node2:setAlign(kAlignLeft);
--    self.m_radioButtonGroup:addChild(node2)

--    self.m_btnT = new(Button,"common/background/activity_bg.png");
--    self.m_btnT:setPos(300,200)
--    self.m_btnT:setAlign(kAlignTop);
--    self.m_root:addChild(self.m_btnT);
--    self.m_btnT:setOnClick(self,function()
--        local ret = self.m_radioButtonGroup:getResult();
--        if ret then
--            print_string("radiobuttongroup index is ------------------->" .. ret);
--        else

--             print_string("radiobuttongroup index is -------------------> null" );
--        end
--    end);
    -----------
...
OnlineScene.entryOnlineGame = function(self,index)
	...
	local isCanEntry = RoomProxy.getInstance():canAccessRoom(index,money);
	...
	local message = "";
	local room = RoomConfig.getInstance():getRoomTypeConfig(index);
    local canEntryRoom = RoomProxy.getInstance():getMatchRoomByMoney(money)
	if canEntryRoom == nil then
...
OnlineScene.onUpdateRoomPlayers = function(self, room_player)
    ...
    if room_player then
        ...
        self.m_arena_room_btn:getChildByName("online_num_bg"):getChildByName("online_num"):setText("当前在线:"..(room_player[4] or 0));
        UserInfo.getInstance():setRoomPlayerNum(room_player);
        self.m_watch_btn:getChildByName("info"):setText("在线:"..(room_player[5] or 0).."人");
    else
        Log.i("房间人数获取失败");
        ...
        self.m_arena_room_btn:getChildByName("online_num_bg"):getChildByName("online_num"):setText("当前在线:0");
    end
end

OnlineScene.onUpdatePrivateRoomNum = function(self,info)
    self.m_private_btn:getChildByName("info"):setText("空闲:"..(info.private_room_num or 0).."间");
end
...
OnlineScene.s_controlConfig = 
{
    ...
    [OnlineScene.s_controls.arena_room_btn]         = {"content_view","arena_room_btn"};
