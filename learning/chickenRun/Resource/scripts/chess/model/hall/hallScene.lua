...
--    self:resumeAnimStart();
    ...
    BottomMenu.getInstance():setHandler(self,BottomMenu.HALLTYPE);
...
--    self.m_hall_union_btn = self:findViewById(self.m_ctrls.hall_union_btn);
    ...
    self.m_hall_rank_bg = self:findViewById(self.m_ctrls.rank_btn_bg);
    ...
    if kPlatform == kPlatformIOS then
        ...
        if tonumber(UserInfo.getInstance():getIosAuditStatus()) ~= 0 then
            ...
            self.m_hall_rank_bg:setVisible(true);
            ...
            self.m_hall_rank_bg:setVisible(false);
...
HallScene.resumeAnimStart = function(self,lastStateObj,timer)
    ...
    if BottomMenu.getInstance():checkState(lastStateObj.state) then
        if not BottomMenu.getInstance():checkStateSort(States.Hall,lastStateObj.state) then
    ...
    local lw,lh = self.m_leaf_left:getSize();
    self.m_leaf_left:addPropTranslate(1,kAnimNormal,waitTime,delay,- lw,0,-30,0);
    local rw,rh = self.m_leaf_right:getSize();
    local anim = self.m_leaf_right:addPropTranslate(1,kAnimNormal,waitTime,delay,rw,0,-30,0);
    if anim then
...
--    self.m_root:removeProp(1);
    ...
    if BottomMenu.getInstance():checkState(newStateObj.state) then
        if BottomMenu.getInstance():checkStateSort(States.Hall,newStateObj.state) then
            self.m_root:removeProp(1);
    ...
    local delay = waitTime+duration;
    local lw,lh = self.m_leaf_left:getSize();
    self.m_leaf_left:addPropTranslate(1,kAnimNormal,waitTime,-1,0,-lw,0,-30);
    local rw,rh = self.m_leaf_right:getSize();
    local anim = self.m_leaf_right:addPropTranslate(1,kAnimNormal,waitTime,-1,0,rw,0,-30);
    if anim then
        anim:setEvent(self,function()
...
--    local is_vip = UserInfo.getInstance():getIsVip();
    ...
    if frameRes.frame_res then
        self.m_vip_frame:setFile(string.format(frameRes.frame_res,130));
    end
end
...
HallScene.showExitDlg = function(self)
	...
	self.leaveTipsDialog:setCoinsListener(self,self.onActivityBtnClick)
...
function HallScene:onHallRandBtnClick()
    StatisticsManager.getInstance():onCountToUM(HALL_MODEL_RANK_BTN);
...
function HallScene:onActivityBtnClick(status)
    StatisticsManager.getInstance():onCountToUM(HALL_MODEL_DAILY_ACTIVITY_BTN);
...
function HallScene:showUpdateGuideDialog()
    ...
    self.m_secondLogGuideDialog:setShowStatus(1);
    ...
    if chatRoomList then
        for i = 1, #chatRoomList do
            ...
            if chatRoomList[i].id == 1001 then
                id = UserInfo.getInstance():getCityCode();
            else
...
HallScene.onShowDailyTask = function(self)
    ...
    if not daily_task_data or type(daily_task_data) ~= "table" then
        return
    end
    ...
    if DailyTaskData.getInstance():getCompleteStatus() then
        self.m_hall_activity_btn:getChildByName("pos"):setVisible(true);
    else
        self.m_hall_activity_btn:getChildByName("pos"):setVisible(false);
    end
end
...
HallScene.s_controlConfig = 
{
    ...
    [HallScene.s_controls.rank_btn_bg]              = {"rank_btn_bg"};
    [HallScene.s_controls.rank_btn]                 = {"rank_btn_bg","rank_btn"};
...
--    [HallScene.s_controls.hall_union_btn]           = HallScene.onHallUnionBtnClick;
    ...
    [HallScene.s_controls.rank_btn]                 = HallScene.onHallRandBtnClick;
