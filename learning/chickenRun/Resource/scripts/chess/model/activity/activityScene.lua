...
function ActivityScene:initView()
    ...
    if dailyTaskIsClear then
        self.m_daily_task_hint:setVisible(true);
    end
...
function ActivityScene:showDailyTask()
    ...
    self.m_daily_task_hint:setVisible(false);
    local label = true;
    ...
    if self.m_data.status == 0 and tonumber(self.m_data.jump)~= 0 then 
        ...
        if self.m_data.id == 17 then
            ...
            if next(data) ~= nil then
                if kPlatform == kPlatformIOS then
                    ActivityScene.m_PayInterface = PayUtil.getPayInstance(PayUtil.s_payType.Exchange);
		            data.position = data.id;
		            ActivityScene.m_pay_dialog = ActivityScene.m_PayInterface:buy(data,data.position);
                else
                    ActivityScene.m_PayInterface = PayUtil.getPayInstance(PayUtil.s_defaultType);
		            data.position = MALL_COINS_GOODS;
		            ActivityScene.m_pay_dialog = ActivityScene.m_PayInterface:buy(data,MALL_COINS_GOODS);
                end
                return
            end
        end
