...
function FindController.isForbidPlayOnline(self)
    -- 封号状态
...
function FindController.onEntryRoom(self, index, flag)
    RoomProxy.getInstance():gotoTypeRoom(index);
end
...
function FindController:quickPlay()
    ...
    if not gotoRoom then 
	    ...
	    self:onEntryRoom(tonumber(gotoRoom.room_type));
    end
end
