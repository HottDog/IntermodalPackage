...
ReplayRoomScene.initUsersInfo = function(self, data)
    ...
    local up_data = (tonumber(self.m_data.down_user) == tonumber(self.m_data.red_mid)) and (self.black_is_vip) or (self.red_is_vip);
    ...
    local tw,th = text:getSize();
    local down_data = (tonumber(self.m_data.down_user) == tonumber(self.m_data.red_mid)) and (self.red_is_vip) or (self.black_is_vip);
...
--        self.m_down_vip_frame:setVisible(false);
    ...
    self.m_down_vip_frame:setVisible(frameRes.visible);
    local fw,fh = self.m_down_vip_frame:getSize();
    if frameRes.frame_res then
        self.m_down_vip_frame:setFile(string.format(frameRes.frame_res,fw));
    end
end;
...
ReplayRoomScene.loadChessData = function(self)
...
--    require(BASE_PATH.."chessShareManager");
    ...
    manualData.title = "复盘演练（博雅中国象棋）";
    manualData.description = "复盘让您回顾精彩对局";
...
ReplayRoomScene.onCollectionBtnClick = function(self)
    ...
    self.m_chioce_dialog:setMessage("是否花费"..500 .."金币收藏当前棋谱？");
