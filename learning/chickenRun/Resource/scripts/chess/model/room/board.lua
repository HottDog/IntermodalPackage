...
--    self:console_gameover(self.m_model + 1,ENDTYPE_KILL);--单机测试,注释下面的代码
...
Board.console_gameover = function(self,code,endType)
	...
	if endType == ENDTYPE_KILL then
		AnimKill.play(self,self.m_room,self.m_room.showResultDialog);
	elseif  endType == ENDTYPE_TIMEOUT then
		AnimTimeout.play(self,self.m_room,self.m_room.showResultDialog);
	elseif endType == ENDTYPE_JAM then
		AnimJam.play(self,self.m_room,self.m_room.showResultDialog);
...
Board.hintMove = function(self,mv)
    ...
    -- 创建残局提示按钮可用了
    EventDispatcher.getInstance():dispatch(Event.Call,CUSTOMENGATE_TIPS_ENABLE);  
end
