...
--    end
    ...
    if self.bindTabitem and #self.bindTabitem > 0 then
        ...
        for i = 1,#self.bindTabitem do
            self.bindTabitem[i]:addPropTransparency(2,kAnimNormal,waitTime-150,(delay + 700 + i * 100) ,0,1);
            self.bindTabitem[i]:addPropScale(1,kAnimNormal,waitTime-150,(delay + 700 + i * 100),0.8,1,0.8,1,kCenterXY,98,110);
        end
    end
    --需要所有动画播完后再调用
    self.timerAnim = new(AnimInt,kAnimNormal,0,1,waitTime-120,delay + 700 + startAnimIntDelay);
...
OwnScene.updateUserInfoView = function(self,data)
    -- 昵称
...
--    self.m_my_setting:getChildByName("content"):setText();
    ...
    --更新绑定按钮状态
    self:updateBindItemStatus(data);
...
function OwnScene.updateBindItemStatus(self,data)
    ...
    for k,v in pairs(self.bindTabitem) do
        if v then
            if v.accountType  == data.sid then
...
--    [OwnScene.s_controls.switch_account]           = {"scroll_view","switch_account"};
    ...
    [OwnScene.s_controls.my_gift]                  = {"scroll_view","gift_view","my_gift"};
