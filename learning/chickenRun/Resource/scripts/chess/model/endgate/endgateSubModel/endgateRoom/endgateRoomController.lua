...
EndgateRoomController.onLoadEndingBoard = function(self)
    ...
    GameCacheData.getInstance():saveInt(GameCacheData.ENDGAME_LATETST_SELECT_TID .. uid,gate.tid);
    GameCacheData.getInstance():saveInt(GameCacheData.ENDGAME_LATETST_SELECT_POS .. uid,gate_sort);
...
EndgateRoomController.showResultDialog = function(self)
	if self.m_boradCode == CHESS_MOVE_OVER_RED_WIN then
        ...
        local uid = UserInfo.getInstance():getUid();
        GameCacheData.getInstance():saveInt(GameCacheData.ENDGAME_LATETST_SELECT_TID .. uid,gate.tid);
        GameCacheData.getInstance():saveInt(GameCacheData.ENDGAME_LATETST_SELECT_POS .. uid,gate_sort+1);
	else
        self:upPhpInfo(0);
        self:updateView(EndgateRoomScene.s_cmds.showEndingResultDialog,self.m_boradCode);
	end
...
--    local url = "http://h5.oa.com/chess/?type=1&id="..id;--测试服
    ...
    data.url = url;
--    data.share_type = "wulin_booth";
--    if kPlatform == kPlatformIOS then
--        data.title = "残局挑战";
    local title = "残局：" .. kEndgateData:getBoardTableSubTitle() .. "(博雅中国象棋)";
    data.title = title;
...
--        share_text_msg(json.encode(data));
    ...
    return data;
--        self:updateView(EndgateRoomScene.s_cmds.shareDialog,data);
--    end;
--    share_text_msg(json.encode(data));
end
...
EndgateRoomController.onRecvShareDialogHide = function(self)
    self:updateView(EndgateRoomScene.s_cmds.shareDialogHide);
end
...
EndgateRoomController.aiUnchangeMove = function(self,flag,data)
    ...
    -- 因为和棋不弹窗 所以这里做特殊判断
    self:updateView(EndgateRoomScene.s_cmds.showEndingResultDialog,0);
end
...
EndgateRoomController.exchangePropCallBack = function(self,isSuccess,message)
    ...
    local payRecommend = MallData.getInstance():getPayRecommend()
	if status == -2 then
...
EndgateRoomController.s_httpRequestsCallBackFuncMap  = {
    ...
    [HttpModule.s_cmds.exchangeProp]       = EndgateRoomController.exchangePropCallBack;
};

EndgateRoomController.s_httpRequestsCallBackFuncMap = CombineTables(ChessController.s_httpRequestsCallBackFuncMap,
	EndgateRoomController.s_httpRequestsCallBackFuncMap or {});
...
EndgateRoomController.s_nativeEventFuncMap = {
    ...
    [kShareDialogHide] = EndgateRoomController.onRecvShareDialogHide;
};
