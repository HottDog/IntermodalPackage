...
ChessController.onHallMsgLogin = function(self, packageInfo)
    ...
    if roomConfig and packageInfo.tid > 0 then 
        local roomProxy = RoomProxy.getInstance();
	    UserInfo.getInstance():setRelogin(true);
        ChessDialogManager.dismissAllDialog();
        if roomConfig:getRoomTypeConfig(RoomConfig.ROOM_TYPE_PRIVATE_ROOM) and packageInfo.level == roomConfig:getRoomTypeConfig(RoomConfig.ROOM_TYPE_PRIVATE_ROOM).level then--重连私人房
            UserInfo.getInstance():setCustomRoomID(packageInfo.tid);
            roomProxy:gotoPrivateRoom();
        elseif roomConfig:getRoomTypeConfig(RoomConfig.ROOM_TYPE_NOVICE_ROOM) and packageInfo.level == roomConfig:getRoomTypeConfig(RoomConfig.ROOM_TYPE_NOVICE_ROOM).level then--初级场  
            roomProxy:gotoNoviceRoom();        
        elseif roomConfig:getRoomTypeConfig(RoomConfig.ROOM_TYPE_INTERMEDIATE_ROOM) and packageInfo.level == roomConfig:getRoomTypeConfig(RoomConfig.ROOM_TYPE_INTERMEDIATE_ROOM).level then--中级场
            roomProxy:gotoIntermediateRoom(); 
        elseif roomConfig:getRoomTypeConfig(RoomConfig.ROOM_TYPE_MASTER_ROOM) and packageInfo.level == roomConfig:getRoomTypeConfig(RoomConfig.ROOM_TYPE_MASTER_ROOM).level then--大师场
            roomProxy:gotoMasterRoom(); 
        elseif roomConfig:getRoomTypeConfig(RoomConfig.ROOM_TYPE_FRIEND_ROOM) and packageInfo.level == roomConfig:getRoomTypeConfig(RoomConfig.ROOM_TYPE_FRIEND_ROOM).level then--好友场
            roomProxy:gotoFriendRoom();
        elseif roomConfig:getRoomTypeConfig(RoomConfig.ROOM_TYPE_ARENA_ROOM) and packageInfo.level == roomConfig:getRoomTypeConfig(RoomConfig.ROOM_TYPE_ARENA_ROOM).level then--好友场
            roomProxy:goteArenaRoom();
        end
...
ChessController.onServerOfflineReconnected = function(self, socket)
   ...
   if socket.m_socketName == "hall" then
    ...
    elseif socket.m_socketName == "room" then
	    if RoomProxy.getInstance():getTid() and RoomProxy.getInstance():getTid() ~= 0 then
...
ChessController.onFriendCmdCheckUserData = function(self,isSuccess,message)
    ...
    FriendsData.getInstance():onCheckUserData(data);
end
