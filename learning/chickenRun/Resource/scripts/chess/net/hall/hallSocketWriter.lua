...
HallSocketWriter.onHallMsgGamePlay = function(self,packetId,info)
    self.m_socket:writeInt(packetId, 4);--房间数;
    ...
    if noviceData and noviceData.level then--新手场;
        self.m_socket:writeInt(packetId, noviceData.level);
    else
        self.m_socket:writeInt(packetId, 0);
    end
    if intermediateData and intermediateData.level then--中级场;
        self.m_socket:writeInt(packetId, intermediateData.level);
    else
        self.m_socket:writeInt(packetId, 0);
    end
    if masterData and masterData.level then--大师场;
        self.m_socket:writeInt(packetId, masterData.level);
    else
        self.m_socket:writeInt(packetId, 0);
    end
    if arenaData and arenaData.level then--竞技场;
        self.m_socket:writeInt(packetId, arenaData.level);
    else
        self.m_socket:writeInt(packetId, 0);
    end
end
...
HallSocketWriter.onHallMsgResponseServer = function(self, packetId, info)
    self.m_socket:writeInt(packetId, RoomProxy.getInstance():getUid());     --用户ID
...
--    local packetId = self.m_socket:writeBegin(CLIENT_WATCH_LOGIN,MAIN_VER, SUB_VER,DEVICE_TYPE);
	...
	self.m_socket:writeShort(packetId,RoomProxy.getInstance():getOBSvid());   --观战服务器ID
	self.m_socket:writeShort(packetId,RoomProxy.getInstance():getTid()); --观战棋桌ID 
...
--	self.m_socket:writeEnd(packetId);
    ...
    Log.i("======房间观战登陆数据发送完成====== tid =" ..RoomProxy.getInstance():getTid() );
...
HallSocketWriter.onSendClientMsgStart = function(self, packetId, info)
	...
	self.m_socket:writeInt(packetId,UserInfo.getInstance():getUid());  --用户id
	self.m_socket:writeShort(packetId,RoomProxy.getInstance():getTid()); --棋桌ID
	self.m_socket:writeShort(packetId,UserInfo.getInstance():getSeatID());   --座位ID    
...
HallSocketWriter.onClientHallCancelMatch = function(self, packetId)
    ...
    if RoomConfig.getInstance():getRoomTypeConfig(roomType) then
        self.m_socket:writeInt(packetId, RoomConfig.getInstance():getRoomTypeConfig(roomType).level); 
        self.m_socket:writeInt(packetId, UserInfo.getInstance():getUid());
    end
end
