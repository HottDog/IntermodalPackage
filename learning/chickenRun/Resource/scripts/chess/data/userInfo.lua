...
UserInfo.login = function(self,data)
    ...
    self:setIsReflowUser(aUser.is_reflow_user);
...
--    local info = json.analyzeJsonNode(new_prop_info);
    if ret then
...
function UserInfo.getGiftNum(self,id)
    for k,v in pairs(self.m_giftList) do
        if v then
            if tonumber(v.gift_id) == id  then
                return tonumber(v.gift_num) or 0;
            end
        end
    end
end
