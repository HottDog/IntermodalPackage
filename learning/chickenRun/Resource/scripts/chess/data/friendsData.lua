...
FriendsData.onCheckUserData = function(self,data)
    ...
    for i,v in pairs(data) do
        local item = self:analyzeUserData(v);
...
function FriendsData:analyzeUserData(data)
...
FriendsData.getUserData = function(self,data)
    ...
    local needCheck = {};
    for i,value in ipairs(tab) do
        local status = self.m_cache_user_data[tonumber(value)] or self:analyzeUserData(json.decode(self:getString(GameCacheData.FRIEND_USER_DATA..(tonumber(value)),nil)));
        if status then
